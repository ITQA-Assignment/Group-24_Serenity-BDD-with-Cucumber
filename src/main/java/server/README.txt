...
note:- use this to run on a different port, --server.port=8081
...
You can append requred apiTest path to above url to test APIs.
...
GET /apiTest/books Get All Book list

GET /apiTest/books/{id} get a book [Enter book id to the {id} to retrive book details]


POST /apiTest/books Create a book
...
PUT /apiTest/books/{id} Update a book
...
DELETE /apiTest/books/{id} delete a book [Enter book id to the {id} to delete book]
...
403 Request apiTest call is forbidden
